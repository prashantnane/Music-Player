...
class Player(tk.Frame):
	...
	def track_widgets(self):
		...
		self.songtrack = tk.Label(self.track, font=("Times",16,"bold"),
						bg="white",fg="dark blue")
		self.songtrack['text'] = 'NIRVANA - Music Player'
		self.songtrack.config(width=30, height=1)
		self.songtrack.grid(row=1,column=0,padx=10)

	def control_widgets(self):
		self.loadSongs = tk.Button(self.controls, bg='purple', fg='yellow', font=10)
		self.loadSongs['text'] = 'Load Songs'
		self.loadSongs['command'] = self.retrieve_songs
		self.loadSongs.grid(row=0, column=0, padx=10)

		self.prev = tk.Button(self.controls, image=prev, border=0)
		self.prev['command'] = self.prev_song
		self.prev.grid(row=0, column=1)

		self.pause = tk.Button(self.controls, image=pause, border=0)
		self.pause['command'] = self.pause_song
		self.pause.grid(row=0, column=2)

		self.next = tk.Button(self.controls, image=next_, border=0)
	...
	def tracklist_widgets(self):
		self.yscrollbar = tk.Scrollbar(self.tracklist, orient=tk.VERTICAL)
		self.yscrollbar.grid(row=0,column=1, rowspan=5, sticky='ns')

		self.xscrollbar = tk.Scrollbar(self.tracklist, orient=tk.HORIZONTAL)
		self.xscrollbar.grid(row=5, column=0, rowspan=5, sticky='we')


		self.list = tk.Listbox(self.tracklist, selectmode=tk.SINGLE,
					 yscrollcommand=self.yscrollbar.set,xscrollcommand=self.xscrollbar.set, selectbackground='blue',bg='violet')
		self.enumerate_songs()
		self.list.config(height=22)
		self.list.bind('<Double-1>', self.play_song)

		self.yscrollbar.config(command=self.list.yview)
		self.xscrollbar.config(command=self.list.xview)
	...
	def play_song(self, event=None):
		if event is not None:
			...
			for i in range(len(self.playlist)):
				self.list.itemconfigure(i, bg="violet")
		...
		self.list.itemconfigure(self.current, bg='purple')
	...
	def prev_song(self):
		if self.current > 0:
			...
			self.current = 0
		self.list.itemconfigure(self.current + 1, bg='violet')
		self.play_song()
